...
//struct Sch_list env_sched_list[2];
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    for (i = 0; i < bin_size; i += BY2PG) {
        ...
        if (bin_size - i >= BY2PG) {
            bcopy(bin + i, (void *)page2kva(p)+offset, BY2PG);
        } else {
            bcopy(bin + i, (void *)page2kva(p)+offset, bin_size - i);
        }
        // FIXME: some pages should be read-only.
        page_insert(env->env_pgdir, p, ROUNDDOWN(va + i,BY2PG), PTE_V | PTE_R);
    }
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
    ...
    /*Step 1: alloc a page. */
    if(page_alloc(&p)!=0){
        printf("page_alloc failed in load_icode.\n");
        return;
    }
    ...
    if((r=load_elf(binary,size,&entry_point,e,load_icode_mapper))!=0){
        printf("load_elf failed in load_icode.\n");
        return;
    }
    e->env_status=ENV_RUNNABLE;
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    if(env_alloc(&e,0)<0){
        printf("env_alloc failed in env_create_priority.\n");
        return;
    }
...
//    LIST_INSERT_HEAD(&env_sched_list[0],e,env_sched_link);
//    TAILQ_INSERT_HEAD(&env_sched_list[0],e,env_sched_link);
}
/* Overview:
...
//        TAILQ_REMOVE(&env_sched_list[list_id],e,env_sched_link);
}
