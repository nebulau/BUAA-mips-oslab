...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
       ...
       if (va >= UTOP || va < 0) {
        ...
        if ((perm & PTE_COW) || !(perm & PTE_V)) {	//cow±ØÐëÊÇ0£¬ÓÐÐ§Î»±ØÐëÊÇ1 
...
void sys_ipc_recv(int sysno, u_int dstva)
{
    ...
    //    LIST_REMOVE(curenv, env_sched_link);  // Lab4¿ÉÓÐ¿ÉÎÞ
        sys_yield();
}
 ...
 * ½«value´«¸øÄ¿±ê½ø³Ìenvid¡£
 *      Èç¹ûÄ¿±ê½ø³ÌÉÐÎ´´¦ÓÚ¿É½ÓÊÕ×´Ì¬£¬·µ»ØÖµÓ¦µ±Îª-E_IPC_NOT_RECV¡£
 *      ÆäËûÇé¿öÏÂ£¬·¢ËÍ³É¹¦ºó£¬Ä¿±ê½ø³ÌµÄIPC²¿·ÖÊý¾ÝÓ¦µ±°´ÕÕÈçÏÂ¹æÔò¸üÐÂ£º
 *      env_ipc_recvingÉèÖÃÎª0£¬·ÀÖ¹¶àÓàµÄ½ÓÊÕ¡£
 *      env_ipc_fromÉèÖÃÎª·¢ËÍ½ø³ÌµÄid¡£
 *      env_ipc_valueÉèÖÃÎªº¯Êý²ÎÊývalue¡£
 *      Ä¿±ê½ø³ÌÐèÒª±ê¼ÇÎªRUNNABLE£¬ÒÔ±ãÖØÐÂÔËÐÐ¡£
                                         ...
                                         u_int perm)
{
       ...
       // LIST_INSERT_HEAD(&env_sched_list[0], e, env_sched_link);  // Lab4¿ÉÓÐ¿ÉÎÞ
